import "flowbite";
export default function NavbarLogin() {
  return (
    <div className=" shadow-lg">
      <nav className="bg-white border-gray-200 ">
        <div className="w-screen flex flex-wrap  items-center justify-between mx-auto p-4">
          <a
            href="https://flowbite.com/"
            className="flex items-center space-x-3 rtl:space-x-reverse"
          >
            {/* <h1 className="text-green-900 text-xl font-medium">☘️FRESH</h1> */}
            <span className="self-center text-2xl font-semibold whitespace-nowrap">
              <h1 className="text-green-900 text-xl font-medium">☘️FRESH</h1>
            </span>
          </a>
          <button
            data-collapse-toggle="navbar-default"
            type="button"
            className="inline-flex items-center  p-2 w-10 h-10 justify-center text-sm text-gray-500 rounded-lg md:hidden hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-gray-200 dark:text-gray-400 dark:focus:ring-gray-600"
            aria-controls="navbar-default"
            aria-expanded="false"
          >
            <span className="sr-only">Open main menu</span>
            <svg
              className="w-5 h-5"
              aria-hidden="true"
              xmlns="http://www.w3.org/2000/svg"
              fill="none"
              viewBox="0 0 17 14"
            >
              <path
                stroke="currentColor"
                strokeLinecap="round"
                strokeLinejoin="round"
                strokeWidth="2"
                d="M1 1h15M1 7h15M1 13h15"
              />
            </svg>
          </button>
          <div className="hidden w-full md:block md:w-auto" id="navbar-default">
            <ul className="font-medium flex flex-col p-4 md:p-0 mt-4 border border-gray-100 roundedLg  md:flex-row md:space-x-8 rtl:space-x-reverse md:mt-0 md:border-0  dark:border-gray-700">
              <li>
                <a
                  href="#"
                  className="block py-2 px-3  rounded md:bg-transparent "
                  aria-current="page"
                >
                  à propos
                </a>
              </li>
              <li>
                <a
                  href="#"
                  className="block py-2 px-3 text-gray-900 rounded hover:bg-gray-100 md:hover:bg-transparent"
                >
                  Nos producteur
                </a>
              </li>
              <li>
                <a
                  href="#"
                  className="block py-2 px-3 text-gray-900 rounded hover:bg-gray-100 md:hover:bg-transparent"
                >
                  Compte
                </a>
              </li>
              <li>
                <a
                  href="#"
                  className="block py-2 px-4 bg-green-400  text-gray-900 rounded-2xl  hover:bg-gray-100 md:hover:bg-transparent"
                >
                  Faire un Partaneriat
                </a>
              </li>
            </ul>
          </div>
        </div>
      </nav>
    </div>
  );
}
